...
use crate::main_gate::{AssignedValue, RegionCtx, MainGate};
...
impl<F: PrimeField, const T: usize> MainGate<F,T> {
    ...
    pub fn assert_equal_const(&self, ctx: &mut RegionCtx<'_, F>, a: AssignedValue<F>, c: F) -> Result<(), Error> {
        self.apply(ctx, (None, None, None), Some(c), (-F::ONE, a.into()))?;
        Ok(())
    }

    // r = 1 <=> zero; r = 0 <=> non-zero
    pub fn invert_with_flag(&self, ctx: &mut RegionCtx<'_, F>, a: AssignedValue<F>) -> Result<(AssignedValue<F>, AssignedValue<F>), Error> {
    ...
    pub fn is_zero_term(&self, ctx: &mut RegionCtx<'_, F>, a: AssignedValue<F>) -> Result<AssignedValue<F>, Error> {
        let (r, _) = self.invert_with_flag(ctx, a)?;
        Ok(r)
    }

    pub fn is_equal_term(&self, ctx: &mut RegionCtx<'_, F>, a: &AssignedValue<F>, b: &AssignedValue<F>) -> Result<AssignedValue<F>, Error> {
        let diff = self.sub(ctx, a, b)?;
        self.is_zero_term(ctx, diff)
    }

    // cond must be either 0 or 1 (e.g. return value from is_zero_term)
    pub fn conditional_select(&self, ctx: &mut RegionCtx<'_, F>, a: &AssignedValue<F>, b: &AssignedValue<F>, cond: &AssignedValue<F>) -> Result<AssignedValue<F>, Error> {
        // res = cond * a + (1-cond) * b
        let state = Some(vec![a.into(), cond.into()]);
        let val1 = cond.value().copied() * a.value().copied();
        let res1 = self.apply(ctx, (None, Some(F::ONE),state), None, (-F::ONE, val1.into()))?;
        
        let state = Some(vec![b.into(), cond.into()]);
        let val2 =  (Value::known(F::ONE) - cond.value().copied()) * b.value().copied();
        let res2 = self.apply(ctx, (Some(vec![F::ONE]), Some(-F::ONE), state), None, (-F::ONE, val2.into()))?;
        self.add(ctx, &res1, &res2)
    }

    // is_inf => 1, otherwise => 0
    pub fn is_infinity_point(&self, ctx: &mut RegionCtx<'_, F>, x: &AssignedValue<F>, y: &AssignedValue<F>) -> Result<AssignedValue<F>, Error> {
    ...
    pub fn add(&self, ctx: &mut RegionCtx<'_, F>, a: &AssignedValue<F>, b: &AssignedValue<F>) -> Result<AssignedValue<F>, Error> {
        // a + b = r
    ...
    pub fn mul(&self, ctx: &mut RegionCtx<'_, F>, a: &AssignedValue<F>, b: &AssignedValue<F>) -> Result<AssignedValue<F>, Error> {
        let state = Some(vec![a.clone().into(), b.clone().into()]);
        let state_terms = (None, Some(F::ONE), state);
        let out_val = a.value().copied() * b.value().copied();
        let out_terms = (-F::ONE, out_val.into());
    ...
    pub fn mul_by_const(&self, ctx: &mut RegionCtx<'_, F>, a: &AssignedValue<F>, b: F) -> Result<AssignedValue<F>, Error> {
    ...
    pub fn square(&self, ctx: &mut RegionCtx<'_, F>, a: &AssignedValue<F>) -> Result<AssignedValue<F>, Error> {
        self.mul(ctx, a, a)
    }

    pub fn divide(&self, ctx: &mut RegionCtx<'_, F>, a: &AssignedValue<F>, b: &AssignedValue<F>) -> Result<AssignedValue<F>, Error> {
