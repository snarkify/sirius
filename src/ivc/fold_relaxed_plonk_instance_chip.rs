    ...
    C::Base: PrimeFieldBits + FromUniformBytes<64>,
{
    ...
    fn fold_via_biguint(
        ...
        debug!(
            ...
            util::CellsValuesView::from(input),
            util::CellsValuesView::from(folded.as_slice()),
            util::CellsValuesView::from(m_bn),
            util::CellsValuesView::from(r_as_bn)
        );
        ...
        debug!(
            "fold: mult mod: {:?}",
            util::CellsValuesView::from(part_mult_r.as_slice())
        );
        ...
        debug!(
            "fold: assign_sum {:?}",
            util::CellsValuesView::from(part_mult_r_sum_part.cells.as_slice())
        );
    ...
    fn fold_instances(
        ...
        debug!(
            "fold: X0 folded: {:?}",
            util::CellsValuesView::from(new_folded_X0.as_slice())
        );
        ...
        debug!(
            "fold: X1 folded: {:?}",
            util::CellsValuesView::from(new_folded_X1.as_slice())
        );
...
mod tests {
    ...
    fn fold_instances_test() {
        ...
        for _round in 0..=NUM_OF_FOLD_ROUNDS {
            ...
            // Run twice for setup & real run
            for _ in 0..=1 {
                on_circuit_instances_cell = Some(
                    layouter
                        .assign_region(
                            || "fold instances test",
                            |region| {
                                ...
                                let assigned_fold_instances = relaxed_plonk
                                    ...
                                    .iter()
                                    .map(|instance| {
                                        assign_scalar_as_bn!(&mut ctx, instance, "folded instance")
                                    })
                                    .collect::<Result<Vec<_>, _>>()
                                    .unwrap()
                                    .try_into()
                                ...
                                let assigned_input_instance = input_instances
                                    .iter()
                                    .map(|instance| {
                                        assign_scalar_as_bn!(&mut ctx, instance, "input instance")
                                    })
                                    .collect::<Result<Vec<_>, _>>()?
                                    .try_into()
                                    .unwrap();
